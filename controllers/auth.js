...
router.post('/sign-up', async (req, res) => {
  try {
    const { username, password, confirmPassword } = req.body // destructure form entries
    ...
    // creates a user property in the session object that saves username
    // and id of the logged in user
    req.session.user = {     
      username: user.username,
      _id: user._id,
    }

    // saves the session into mongoDB (becasue sessions are saved async)
    // and rediret them to some page
    req.session.save(() => { 
      res.redirect('/')
    })
  } catch (error) {
